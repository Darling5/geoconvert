# -*- coding: utf-8 -*-
"""Gitee Release v1.5.11 发布脚本（凭据从 Windows 凭据管理器读取，不落盘）。

用法: python tools/gitee_release_1511.py
...
def create_release(tok):
    ...
    data = json.dumps({'access_token': tok, 'tag_name': TAG,
                       'name': 'geoconvert %s — 支持新版 ContextCapture OSGB（OSG v131），局部坐标不再报错' % TAG,
...
def main():
    if not SHA:
        ...
        sha = p.stdout.strip()
        if not sha:
            raise SystemExit('SHA 为空且 git rev-parse 失败')
        globals()['SHA'] = sha
    tok = gitee_token()
    print('令牌已取得（不入库）')
